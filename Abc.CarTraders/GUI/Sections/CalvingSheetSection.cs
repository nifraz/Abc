...
{
    public partial class CalvingSheetSection : UserControl, IColoredControl
    {
        #region Common
        ...
        public async Task ExportToExcelAsync()
        {
            ...
            while (result == DialogResult.Retry)
            {
                try
                {
                    ...
                    var path = await UnitOfWork.CalvingSheets.ExportToExcelAsync(RangeField, RangeStart, RangeEnd, CalvingSheetProvince, CalvingSheetDistrict, CalvingSheetVsRange, CalvingSheetInstitute, CalvingSheetTechnicianCode, SortField, SortDirection);
                    StopProgress();
        ...
        private IList<CalvingSheet> SelectedRecords
        {
            get
            {
                return dataGridView1.SelectedRows.Cast<DataGridViewRow>().Select(dgvr => dgvr.DataBoundItem as CalvingSheet).ToList();
            }
        }

        private CalvingSheet SelectedRecord
        {
            get
            {
                return dataGridView1.CurrentRow.DataBoundItem as CalvingSheet;
            }
        }
        ...
        private void DeleteRecords()
        {
            ...
            var calvingSheets = SelectedRecords.OrderBy(cs => cs.Id);
            var calvingSheetCount = calvingSheets.Count();
            var calvingSheetIds = string.Join(",", calvingSheets.Select(cs => cs.Id));
            var calvingRecordCount = calvingSheets.SelectMany(cs => cs.CalvingRecords).Count();
            ...
            WriteLog.Invoke(new Log()
            {
                ...
                Text = $"Deleted {calvingSheetCount} calving sheet(s) (#{calvingSheetIds}) (with {calvingRecordCount} calving record(s))"
            });
        }
        ...
        private IPagedList<CalvingSheet> PagedList { get; set; }
        ...
        public async Task RefreshAsync()
        {
            ...
            while (result == DialogResult.Retry)
            {
                try
                {
                    ...
                    PagedList = await UnitOfWork.CalvingSheets.GetPagedListAsync(RangeField, RangeStart, RangeEnd, CalvingSheetProvince, CalvingSheetDistrict, CalvingSheetVsRange, CalvingSheetInstitute, CalvingSheetTechnicianCode, SortField, SortDirection, PageNumber, PageSize);
                    StopProgress();

                    dataGridView1.DataSource = new BindingList<CalvingSheet>(PagedList.ToList());
        ...
        #region Filter
        private Province CalvingSheetProvince
        {
            get
            {
        ...
        private District CalvingSheetDistrict
        {
            get
            {
        ...
        private VsRange CalvingSheetVsRange
        {
            get
            {
        ...
        private Institute CalvingSheetInstitute
        {
            get
            {
        ...
        {
            if (CalvingSheetProvince == null && CalvingSheetDistrict == null && CalvingSheetVsRange == null && CalvingSheetInstitute == null && CalvingSheetTechnicianCode == null) return;

            CalvingSheetProvince = null;
